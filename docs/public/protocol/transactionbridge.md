...
The TransactionBridge is a genesis deployed contract that can be called with typed signature conforming to EIP-712 (Typed Structured Data Hashing and Signing).
...
interface ITransactionBridge {
    ...
    function submitWithAuthorisation(Txn calldata txn, bytes calldata signature) external payable;
